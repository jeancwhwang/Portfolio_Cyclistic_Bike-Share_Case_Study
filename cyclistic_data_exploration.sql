...
SELECT julianday(ended_at)-julianday(started_at) as julianday_diff
FROM cyclistic
...
SELECT	start_lat||' '|| start_lng AS start_coordinates,
	COUNT(*),
	(SELECT COUNT(*) FROM cyclistic) AS Total,
...
SELECT	end_lat||' '|| end_lng AS end_coordinates,
	COUNT(*),
	(SELECT COUNT(*) FROM cyclistic) AS Total,
	COUNT(*)*100/(SELECT COUNT(*) FROM cyclistic) AS Pct_start_coordinates
FROM cyclistic
